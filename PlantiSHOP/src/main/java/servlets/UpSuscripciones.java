...
public class UpSuscripciones extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            try (Connection c = DriverManager.getConnection("jdbc:mysql://localhost/PSHOP", "root", "n0m3l0")) {
                try (Statement s = c.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
                    ...
                    switch (request.getParameter("ps")) {
                        case "25.00": {
                            s.executeUpdate("UPDATE Usuario SET P=TRUE,FfS='"+LocalDate.now().plusDays(30L).toString()+"' WHERE CE='"+request.getSession().getAttribute("u")+"'");
                        } break;
                        case "75.00": {
                            s.executeUpdate("UPDATE Usuario SET P=TRUE,FfS='"+LocalDate.now().plusMonths(3L).toString()+"' WHERE CE='"+request.getSession().getAttribute("u")+"'");
                        } break;
                        case "150.00": {
                            s.executeUpdate("UPDATE Usuario SET P=TRUE,FfS='"+LocalDate.now().plusMonths(6L).toString()+"' WHERE CE='"+request.getSession().getAttribute("u")+"'");
                        } break;
                        case "300.00": {
                            s.executeUpdate("UPDATE Usuario SET P=TRUE,FfS='"+LocalDate.now().plusYears(1L).toString()+"' WHERE CE='"+request.getSession().getAttribute("u")+"'");
                        } break;
                    }
                    s.execute("COMMIT");
                }
